...
def verso_virata(rotta_prima, rotta_dopo):
    """Se il cambio di direzione da rotta_prima a rotta_dopo è verso
    destra o verso sinistra (visto dall'alto, nel senso di marcia)."""
    diff = (rotta_dopo - rotta_prima + 180) % 360 - 180
    return "destra" if diff > 0 else "sinistra"
...
def conta_strambate(
    ...
    for i in range(n):
        ...
        if angolo >= soglia_angolo:
            verso = verso_virata(rotta_prima, rotta_dopo)
            candidati.append([i, d_i, angolo, verso])
    ...
    dettagli = []
    for indice, d_evento, angolo, verso in eventi:
...
def estrai_punti_traccia(dettagli):
    """Estrae lat/lon/velocità punto per punto da 'dettagli' (il risultato
    di get_activity_details), usati per la mappa, le strambate e il
    grafico di confronto tra uscite."""
    punti = []
    if not dettagli:
        return punti
    try:
        ...
        for p in dettagli.get("activityDetailMetrics", []):
            ...
            if lat is not None and lon is not None:
                punti.append({"lat": lat, "lon": lon, "velocita_kmh": (vel or 0) * 3.6})
    except Exception:
        return []
    return punti


@st.cache_data(show_spinner="Calcolo il confronto con le altre uscite...", ttl=600)
def calcola_storico_prestazioni(
    id_e_date, soglia_velocita_kmh=10, finestra_m=30, finestra_rotta_m=25, soglia_angolo=140, limite=20
):
    """Per un elenco di (activity_id, data) calcola velocità massima e
    numero di strambate riuscite di ciascuna uscita, per confrontarle in
    un grafico. Limitato alle uscite più recenti per non sovraccaricare
    Garmin di richieste (una per uscita)."""
    righe = []
    for activity_id, data_attivita in id_e_date[-limite:]:
        try:
            riepilogo, dettagli, _ = carica_dettaglio_attivita(activity_id)
        except Exception:
            continue
        ...
        if len(punti) >= 3:
            try:
                numero_strambate, _ = conta_strambate(
                    punti,
                    soglia_velocita_kmh=soglia_velocita_kmh,
                    finestra_m=finestra_m,
                    finestra_rotta_m=finestra_rotta_m,
        ...
        righe.append(
            {
                "data": data_attivita,
                "velocita_max_kmh": round(velocita_max * 3.6, 1) if velocita_max is not None else None,
                "numero_strambate": numero_strambate,
            }
        )
...
def mostra_grafico_confronto(sotto_tabella_sport):
    ...
    questo sport — solo per windsurf/wing foil e simili."""
    id_e_date = tuple(
        sorted((int(r["id"]), r["data"]) for _, r in sotto_tabella_sport.iterrows())
    )
    if not id_e_date:
        return
    storico = calcola_storico_prestazioni(id_e_date)
    if storico.empty:
        return
...
# =========================================================
    ...
    # Sappiamo già da qui se è uno sport di vela/foil, così possiamo
    # personalizzare le statistiche mostrate più sotto.
    is_vela_o_foil = any(parola in str(riga["tipo"]).lower() for parola in PAROLE_CHIAVE_VELA_FOIL)

    st.subheader(f"{riga['data']} — {riga['nome']}")

    if is_vela_o_foil:
        mostra_grafico_confronto(tabella[tabella["tipo"] == riga["tipo"]])
    ...
    if is_vela_o_foil:
        ...
        c2.metric("Velocità media", f"{riga['velocita_media_kmh']} km/h")
        c3.metric(
            "Velocità massima",
    ...
    if riepilogo:
        ...
        if not is_vela_o_foil:
            altri_campi["Altitudine min (m)"] = trova_valore(riepilogo, "minElevation")
            altri_campi["Altitudine max (m)"] = trova_valore(riepilogo, "maxElevation")
    ...
    punti_traccia = estrai_punti_traccia(dettagli)
    ...
    if is_vela_o_foil:
        ...
        if len(punti_traccia) < 3:
            ...
            if dettagli_strambate:
                numero_destra = sum(1 for d in dettagli_strambate if d["lato"] == "destra")
                numero_sinistra = sum(1 for d in dettagli_strambate if d["lato"] == "sinistra")
                st.caption("Dettaglio delle manovre rilevate:")
                st.dataframe(
                    pd.DataFrame(
                        [
                            {"lato": "A destra", "strambate": numero_destra},
                            {"lato": "A sinistra", "strambate": numero_sinistra},
                        ]
                    ),
                    use_container_width=True,
                    hide_index=True,
    ...
    if dettagli:
        try:
            ...
            if is_vela_o_foil:
                metriche_da_mostrare = [
                    m for m in metriche_da_mostrare if m[0] != "directElevation"
                ]

            serie = {}
            for chiave_metrica, nome_leggibile in metriche_da_mostrare:
